...
#let _strip-leading-neg(term) = {
  ...
  if is-type(term, "mul") {
    ...
    if rl != none and rl.n < 0 {
      let mag = rat-neg(rl)
      if rat-eq(mag, rat(1, 1)) { return r }
      return mul(rat-to-expr(mag), r)
    }

    let rr = _as-rat(r)
    if rr != none and rr.n < 0 {
      let mag = rat-neg(rr)
      if rat-eq(mag, rat(1, 1)) { return l }
      return mul(rat-to-expr(mag), l)
    }
  }
...
/// Reads `t` as `u^2` and returns `u`, including perfect-square rationals.
#let _square-base(t) = {
...
#let _difference-of-squares-cancel(n, d) = {
  ...
  let t1 = n.args.at(0)
  let t2 = n.args.at(1)
  let t2-pos = _strip-leading-neg(t2)
  if t2-pos == none { return none }

  let a = _square-base(t1)
  let b = _square-base(t2-pos)
  ...
  if expr-eq(d, a-minus-b) { return add(a, b) }
  if expr-eq(d, b-minus-a) { return neg(add(a, b)) }
  none
}
...
#let _simplify-once(expr) = {
  ...
  if is-type(expr, "div") {
    ...
    // Targeted guarded cancellation for difference-of-squares forms.
    // Restriction handling (`den != 0`) is reported by restriction metadata.
    let dos = _difference-of-squares-cancel(n, d)
    if dos != none {
      return _simplify-once(dos)
    }
...
#let _simplify-fixed-point(expr, allow-domain-sensitive: false) = {
  let current = expr
  let max-passes = 10
...
/// - allow-domain-sensitive: enables identities marked `domain-sensitive`.
///   Keep `false` for branch-safe default behavior.
#let simplify(expr, allow-domain-sensitive: false) = _simplify-fixed-point(
  expr,
  allow-domain-sensitive: allow-domain-sensitive,
)

/// Public helper `simplify-meta-core`.
/// Returns simplified expression with advisory restriction metadata.
#let simplify-meta-core(expr, allow-domain-sensitive: false, assumptions: none) = {
  let simplified = _simplify-fixed-point(expr, allow-domain-sensitive: allow-domain-sensitive)
