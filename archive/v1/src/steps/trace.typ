...
#import "../expr.typ": *
#import "../simplify.typ": simplify, simplify-meta-core
...
#import "../restrictions.typ": render-restriction-note, collect-structural-restrictions, collect-function-restrictions, merge-restrictions, filter-restrictions-by-assumptions, restriction-key
...
/// Keep residual constraints that are still unresolved.
#let _unresolved-residual(filtered) = {
  let unresolved-keys = (:)
  for r in filtered.restrictions {
    unresolved-keys.insert(restriction-key(r), true)
  }
  let out = ()
  for r in filtered.residual {
    if unresolved-keys.at(restriction-key(r), default: false) {
      out.push(r)
    }
  }
  out
...
#let step-diff(expr, var, depth: none, assumptions: none) = {
  _check-free-var(var)
  ...
  let restrictions = merge-restrictions(
    ...
    collect-function-restrictions(expr, stage: "diff"),
  )
  let filtered = filter-restrictions-by-assumptions(restrictions, assumptions)
  let residual-unresolved = _unresolved-residual(filtered)
  if residual-unresolved.len() > 0 {
    steps.push(_s-note("Residual non-affine constraints (not promotable to variable domains):"))
    for r in residual-unresolved {
      steps.push(_s-note(render-restriction-note(r)))
    }
  }
  if filtered.restrictions.len() > 0 {
    steps.push(_s-note("Restrictions carried from original expression:"))
    for r in filtered.restrictions {
      steps.push(_s-note(render-restriction-note(r)))
    }
  } else if assumptions != none and filtered.satisfied.len() > 0 {
    steps.push(_s-note("All domain restrictions satisfied by assumptions."))
  }
  if filtered.conflicts.len() > 0 {
    steps.push(_s-note("Warning: assumptions conflict with one or more domain restrictions."))
...
/// Optional `assumptions` are used to classify restriction metadata.
#let step-integrate(expr, var, depth: none, assumptions: none) = {
  _check-free-var(var)
  let used_init = (var, "f", "g", "h")
  ...
  let restrictions = merge-restrictions(
    ...
    collect-function-restrictions(expr, stage: "integ"),
  )
  let filtered = filter-restrictions-by-assumptions(restrictions, assumptions)
  let residual-unresolved = _unresolved-residual(filtered)
  if residual-unresolved.len() > 0 {
...
/// Optional `assumptions` are used to classify restriction metadata.
#let step-simplify(expr, depth: none, assumptions: none) = {
  let used_init = ("f", "g", "h")
  ...
  let meta = simplify-meta-core(expr, assumptions: assumptions)
  let target = meta.expr
  let current = result
  for i in range(5) {
  ...
  if meta.restrictions.len() > 0 {
    let residual-unresolved = _unresolved-residual(meta)
    if residual-unresolved.len() > 0 {
      steps.push(_s-note("Residual non-affine constraints (not promotable to variable domains):"))
      for r in residual-unresolved {
        steps.push(_s-note(render-restriction-note(r)))
    ...
    steps.push(_s-note("Restrictions carried from original expression:"))
    for r in meta.restrictions {
      steps.push(_s-note(render-restriction-note(r)))
    }
  } else if assumptions != none and meta.satisfied.len() > 0 {
    steps.push(_s-note("All domain restrictions satisfied by assumptions."))
  }
  if meta.conflicts.len() > 0 {
    steps.push(_s-note("Warning: assumptions conflict with one or more domain restrictions."))
    for r in meta.conflicts {
      steps.push(_s-note(render-restriction-note(r)))
    }
  }
