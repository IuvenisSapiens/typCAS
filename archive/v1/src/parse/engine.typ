...
#let _parse-atom(tokens, pos, p) = {
  ...
  if tok.type == "lparen" {
    ...
    if q >= tokens.len() or _pk(tokens, q).type != "rparen" {
  ...
  if tok.type == "ident" {
    ...
    if name == "log" and next != none and next.type == "num" {
      ...
      if (base-num == "2" or base-num == "10") and after-base != none and after-base.type == "lparen" {
        ...
        if q >= tokens.len() or _pk(tokens, q).type != "rparen" {
          panic("cas-parse: malformed log" + base-num + " call, expected closing ')'")
        }
        return (func("log" + base-num, arg), q + 1)
      }
    }
    ...
    if name == "log" and _is-op(tokens, pos + 1, "_") {
      ...
      if next-tok != none and next-tok.type == "lparen" {
        ...
        if q2 >= tokens.len() or _pk(tokens, q2).type != "rparen" {
          panic("cas-parse: malformed log subscript call, expected closing ')'")
        }
    ...
    if next != none and next.type == "lparen" {
      if name == "frac" {
        ...
        if q >= tokens.len() or _pk(tokens, q).type != "comma" {
          panic("cas-parse: malformed frac call, expected ','")
        }
        let (d, q2) = (p.expr)(tokens, q + 1, p)
        if q2 >= tokens.len() or _pk(tokens, q2).type != "rparen" {
          panic("cas-parse: malformed frac call, expected closing ')'")
        }
      ...
      if name == "sqrt" {
        ...
        if q >= tokens.len() or _pk(tokens, q).type != "rparen" {
          panic("cas-parse: malformed sqrt call, expected closing ')'")
        }
      ...
      if name == "root" {
        ...
        let (arg, q2) = (p.expr)(tokens, q + 1, p)
        if q2 >= tokens.len() or _pk(tokens, q2).type != "rparen" {
          panic("cas-parse: malformed root call, expected closing ')'")
        }
        return (pow(arg, cdiv(num(1), index)), q2 + 1)
      }
      if name == "log" {
        ...
        if q < tokens.len() and _pk(tokens, q).type == "comma" {
          ...
          if q2 >= tokens.len() or _pk(tokens, q2).type != "rparen" {
            panic("cas-parse: malformed log(base, arg) call, expected closing ')'")
          }
          return (log-of(first, second), q2 + 1)
        }
        if q >= tokens.len() or _pk(tokens, q).type != "rparen" {
          panic("cas-parse: malformed log call, expected closing ')'")
        }
