...
#{
  ...
  let meta = simplify-meta(expr)
  block(below: 0.6em)[
    *Guarded cancellation with restriction metadata:*
    $ #cas-display(expr) arrow.r.double #cas-display(meta.expr) $
    #if meta.restrictions.len() > 0 [
      #for r in meta.restrictions [
        #if r.rel == "!=" [
          $ "restriction:" #h(0.4em) #cas-display(r.lhs) != #cas-display(r.rhs) $
        ] else if r.rel == ">" [
          $ "restriction:" #h(0.4em) #cas-display(r.lhs) > #cas-display(r.rhs) $
        ] else if r.rel == ">=" [
          $ "restriction:" #h(0.4em) #cas-display(r.lhs) >= #cas-display(r.rhs) $
...
#{
  let a = assume("x", positive: true, nonzero: true)
  let meta = simplify-meta(p("ln(x)"), assumptions: a)
  block(below: 0.6em)[
    *Assumption filtering on restrictions (ln):*
    $ #cas-display(meta.expr) $
    $ "unresolved" = #meta.restrictions.len(), #h(0.8em) "satisfied" = #meta.satisfied.len() $
  ]
}

#{
  let a = assume-domain("x", "(-inf,1) U (1,inf)")
  let meta = simplify-meta(p("(x^2 - 1)/(x - 1)"), assumptions: a)
  block(below: 0.6em)[
    *Domain string exclusion (x != 1):*
    $ #cas-display(meta.expr) $
    $ "unresolved" = #meta.restrictions.len(), #h(0.8em) "satisfied" = #meta.satisfied.len() $
    $ "propagated-domain-vars" = #meta.variable-domains.keys().len() $
  ]
}

#{
  let a = assume-string("x", "(0")
  block(below: 0.6em)[
    *Assumption string shorthand `(0`:*
    $ #cas-display(simplify(p("sqrt(x^2)"), assumptions: a)) $
  ]
}
...
#{
  let expr = p("(x^2 - 1)(x-2)/((x - 1)(x-2))")
  let steps = step-simplify(expr, assumptions: assume("x", positive: true))
  display-steps(expr, steps)
}
